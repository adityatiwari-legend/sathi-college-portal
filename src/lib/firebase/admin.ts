...
const HARDCODED_DATABASE_URL = `https://mysaathiapp-default-rtdb.firebaseio.com/`;
...
function initializeFirebaseAdminApp() {
  ...
  try {
    ...
    if (!defaultInitDatabaseURL && HARDCODED_PROJECT_ID) {
      defaultInitDatabaseURL = `https://${HARDCODED_PROJECT_ID}-default-rtdb.firebaseio.com/`;
    }
    if (!defaultInitDatabaseURL.endsWith('/')) {
    ...
    console.log(`   Using Storage Bucket: ${storageBucket}`);
    console.log(`   Using Database URL: ${defaultInitDatabaseURL}`);
    return;
  } catch (e: any) {
