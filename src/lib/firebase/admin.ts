...
const HARDCODED_STORAGE_BUCKET = "mysaathiapp.firebasestorage.app"; // Usually your_project_id.appspot.com or your_project_id.firebasestorage.app
...
function initializeFirebaseAdminApp() {
  ...
  let databaseURL = process.env.FIREBASE_DATABASE_URL || HARDCODED_DATABASE_URL;
  if (databaseURL && !databaseURL.endsWith('/')) {
      databaseURL += '/';
  }

  console.log(`Firebase Admin SDK: Effective Project ID determined as: ${effectiveProjectId}`);
  if (effectiveProjectId === "YOUR_PROJECT_ID" || effectiveProjectId === "project-sathi-4ad40") {
    console.error("Firebase Admin SDK: CRITICAL - effectiveProjectId is a placeholder or incorrect. Using:", effectiveProjectId);
  }
  console.log(`Firebase Admin SDK: Storage Bucket to be used: ${storageBucket}`);
  console.log(`Firebase Admin SDK: Database URL to be used: ${databaseURL}`);
  ...
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    try {
      ...
      console.log('   Using Storage Bucket (GAC path):', storageBucket);
      console.log('   Using Database URL (GAC path):', databaseURL);
      admin.initializeApp({
        storageBucket,
        databaseURL,
  ...
  if ((usedEnvVars || usedHardcoded) && effectiveProjectId) {
    ...
    try {
      ...
      console.log('   Using Project ID (Cert path):', effectiveProjectId);
      console.log('   Using Storage Bucket (Cert path):', storageBucket);
      console.log('   Using Database URL (Cert path):', databaseURL);
      ...
      admin.initializeApp({
        ...
        projectId: effectiveProjectId, // Ensure projectId is passed
      });
      adminInstance = admin;
  ...
  try {
    ...
    let defaultInitDatabaseURL = databaseURL; // Already includes fallback logic
    if (!defaultInitDatabaseURL && effectiveProjectId) { // Construct if still missing
    ...
    console.log('   Using Project ID (Default ADC path):', effectiveProjectId);
    console.log('   Using Storage Bucket (Default ADC path):', storageBucket);
    console.log('   Using Database URL (Default ADC path):', defaultInitDatabaseURL);
