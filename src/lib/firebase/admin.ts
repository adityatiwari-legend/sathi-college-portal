...
// Use the consistent storage bucket name, ensure it's correct for 'mysaathiapp'
...
function initializeFirebaseAdminApp() {
  ...
  const envStorageBucket = process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET; // Can be used by admin if set
  let envDatabaseURL = process.env.FIREBASE_DATABASE_URL;

  // Try GOOGLE_APPLICATION_CREDENTIALS first (standard for many Google Cloud environments)
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    try {
      ...
      admin.initializeApp({
        ...
        projectId: envProjectId, // Often derived from ADC, but can be specified
      });
      adminInstance = admin;
  ...
  // Try specific FIREBASE_... environment variables
  if (envProjectId && envClientEmail && envPrivateKey) {
    ...
    let effectiveDatabaseURL = envDatabaseURL ? (envDatabaseURL.endsWith('/') ? envDatabaseURL : envDatabaseURL + '/') : HARDCODED_DATABASE_URL;
  ...
  // Hardcoded fallback (already updated with your new key)
  ...
  const serviceAccount = {
    ...
    privateKey: formatPrivateKey(HARDCODED_PRIVATE_KEY), // Already contains the new key
  };
  try {
    admin.initializeApp({
  ...
  try {
    ...
    if (defaultProjectId && !defaultInitDatabaseURL.includes(defaultProjectId)) { 
        defaultInitDatabaseURL = `https://${defaultProjectId}-default-rtdb.firebaseio.com/`;
    }
  ...
  adminInstance = null; // Ensure adminInstance is null if all attempts fail
}
