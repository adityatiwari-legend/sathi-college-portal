...
// Use the exact private key provided by the user
...
const HARDCODED_STORAGE_BUCKET = "mysaathiapp"; // Explicitly use this
...
function initializeFirebaseAdminApp() {
  if (admin.apps.length > 0) {
    adminInstance = admin;
    console.log('Firebase Admin SDK (admin.ts): Already initialized.');
    return;
  }
  ...
  const envStorageBucket = process.env.NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET; // Corrected env var name
  ...
  console.log(`Firebase Admin SDK (admin.ts): Effective Project ID: ${effectiveProjectId}`);
  console.log(`Firebase Admin SDK (admin.ts): Using Storage Bucket: ${storageBucket}`);
  console.log(`Firebase Admin SDK (admin.ts): Using Database URL: ${databaseURL}`);
  ...
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    try {
      ...
      adminInstance = admin;
      console.log('Firebase Admin SDK (admin.ts): Successfully initialized using GOOGLE_APPLICATION_CREDENTIALS.');
      return;
    } catch (e: any) {
  ...
  if ((usedEnvVars || usedHardcoded) && effectiveProjectId) {
    ...
    try {
      ...
      adminInstance = admin;
      console.log(`Firebase Admin SDK (admin.ts): Successfully initialized using ${initMethod}.`);
      return;
    } catch (e: any) {
      console.error(`Firebase Admin SDK (admin.ts): Error initializing with ${usedEnvVars ? "specific FIREBASE_... env vars" : "hardcoded fallback"}:`, e.message);
    }
  } else {
  ...
  try {
    ...
    adminInstance = admin;
    console.log('Firebase Admin SDK (admin.ts): Successfully initialized using default credentials (e.g., App Engine, Cloud Functions).');
    return;
  } catch (e: any) {
